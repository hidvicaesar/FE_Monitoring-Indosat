...
export function Devices() {
  ...
  const { metrics: networkMetrics, loading: networkLoading } =
    useNetworkMetrics();
  ...
  // Helper untuk format lastUpdate timestamp
  ...
  // Gabungkan status ping & waktu ke dalam daftar devices
  const devicesWithRealStatus = devices.map((device) => {
    // Cari data ping perangkat ini dari Network Metrics
    const netStat = networkMetrics.find((m) => m.deviceId === device.id);

    // Jika network status "down", maka device offline
    const realStatus =
      netStat && netStat.status === "down" ? "offline" : "online";

    // Gunakan waktu dari Network jika tersedia,
    // jika tidak gunakan lastSeen dari device
    const displayLastSeen =
      netStat && netStat.lastUpdate && netStat.lastUpdate !== "-"
        ? netStat.lastUpdate
    ...
    <PageContainer>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-white dark:bg-zinc-900 border border-slate-300 dark:border-zinc-800 rounded-lg p-4 shadow-sm">
          <p className="text-sm text-slate-600 dark:text-zinc-500 mb-1">
            Total Devices
          </p>
          ...
          {filteredDevices.map((device) => (
            <DeviceStatusCard
              ...
              onLocationSave={async (newLoc) => {
                if (updateLocation) {
                  await updateLocation(device.id, newLoc);
                }
              }}
            />
          ))}
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-zinc-800">
                {filteredDevices.map((device) => (
                  ...
                  >
                    ...
                    {/* IP Address */}
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-mono text-slate-700 dark:text-zinc-400">
                        {device.ip}
                      </div>
                    </td>
